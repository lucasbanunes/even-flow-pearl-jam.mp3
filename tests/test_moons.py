...
def test_time_embedding_mlp_cnf_torch(data_dir: Path):

    mlflow.set_experiment(f"{__name__}_test_time_embedding_mlp_cnf_torch")

    config = data_dir / 'test_moons' / 'test_time_embedding_mlp_cnf_torch.yaml'
    job = time_embedding_mlp_cnf_torch(config)
    loaded_job = MoonsTimeEmbeddingMLPCNFTorchJob.from_mlflow_run_id(
        job.id_
    )
    job_dict = job.model_dump(exclude='model.lightning_module')
    loaded_job_dict = loaded_job.model_dump(exclude='model.lightning_module')
    ...
    logging.info(f"Loaded job: {loaded_job_dict}")
    assert job_dict == loaded_job_dict, "The loaded job does not match the original job."
    # Ensures that the Torch module is loaded properly.
    assert loaded_job.model.torch_module is not None, "The Torch module was not loaded properly."
...
def test_cli_time_embedding_mlp_cnf_torch(data_dir: Path):

    mlflow.set_experiment(f"{__name__}_test_cli_time_embedding_mlp_cnf_torch")

    config = data_dir / 'test_moons' / 'test_time_embedding_mlp_cnf_torch.yaml'
    result = subprocess.run(
        ['uv', 'run', 'python', 'cli.py',
         'moons',
         'time-embedding-mlp-cnf-torch',
         '--config', str(config)],
        capture_output=True,
        text=True
    )
